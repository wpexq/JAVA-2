...
### 컨테이너와 컴포넌트
- 컨테이너
    - 다른 컴포턴트를 포함할 수 있는 GUI : java.awt.Container을 상속받음
    - 다른 컨테이너에 포함
    - AWT 컨테이너 : Panerl, Frame, Applet, Dialog, Window
    - Swing 컨테이너 : JPanerl, JFrame, JApplet, JDialog, JWindow
- 컴포넌트
    - 컨테이너에 포함되어야 화면에 출력 가능한 GUI 객체
    - 다른 컴포넌트를 포함할 수 없음 (순수 컴포넌트)
    - 모든 GUI 컴포넌트가 상속받는 클래스 : java.awt.Component
    - 스윙 컴포넌트가 상속받는 클래스 : java.swing.JComponent
- 최상위 컨테이너
    - 다른 컨테이너에 포함되지 않고도 화면에 출력, 독독립적으로 존재 가능한 컨테이너
    - 스스로 화면에 자신을 출력
...
- 컨테이너에 새로운 배치관리자 설정
    - setLayout(LayoutManager Im) 메소드 호출
        - Im을 새로운 배치관리자로 설정
- 배치관리자가 없는 컨테이너
    - 배치관리자가 없는 컨테이너가 필요한 경우 - 응용프로그램에서  직접 컴포넌트의 크리와 위치를 결정하고자 하는 경우
        - 크기나 위치를 임의로 결정하는 경우
        - 시간이나 마우스/키보드의 입력에 따라 위치와 크기가 수시로 변하는 경우
        - 겹쳐서 출력하고자 하는 경우
    - 컨테이너 배치관리자 제거 방법
...
- 이벤트 기반 프로그래밍
    - 이벤트의 발생에 의해 프로그램 흐름이 결정되는 방식
        - 이벤트 리스너 실행
    - ㅏ반대되는 개념 : 배치 실행
        - 개발자가 프로그램의 프름을 결정하는 방식
    - 이벤트 종류
        - 사용자 입력 : 마우스 드래그, 클릭, 키보드 누름 etc
        - 센서로부터의 입력, 네트워크로부터 데이터 송수신
        - 다른 응용프로그램이나 스레드로부터의 메시지
    - 이벤트 기반 프로그램의 구조
        - 리스너 코드 보유
    - GUI 응용프로그램은 이벤트 기반 프로그래밍으로 작성
- 이벤트 객체
    - 이벤트 객체
    ...
    - 이벤트 객체가 포함하는 정보
        ...
        - 화면 좌표 및 컴포넌트 내 좌표
        - 버튼이나 메뉴 아이템의 문자열
        - 마우스 버튼 번호 및 마우스의 클릭 횟수
        - 키의 코드 값과 문자 값
        - 체크박스, 라디오버튼 등 이벤트 발생시 체크 상태
...
3. 이벤트 리스너 등록
    - 이베트를 받아 처리하고자 하는 컴포넌트에 이벤트 리스너 등록
    - component.addXXXListener(listener) *XXX:이벤트명
- 이벤트 리스너 작성 방법
    - 독립 클래스로 작성
        - 여러 곳에서 사용할 때 적합
    - 내부 클래스로 작성
...
- Math.random() 메소드로 난수 발생
    - random()은 0보다 크거나 같고 1.9보다 작은 난수 발생
```java
ex) public class MathEx {
    public static void main(String[] args) {
        System.out.println(Math.abs(-3.14));    // 절댓값
        System.out.println(Math.sqrt(9.0));     // 0의 제곱근 =3
        System.out.println(Math.exp(2));        // e제곱
        System.out.println(Math.round(3.14));   // 반올림

        System.out.println("이번 주 행운의 번호는 ");
        for(int i = 0; i < 5; i++)
            System.out.print((int)(Math.radom()* 45 + 1) + " "); // 난수 발생
        }
    }
```
...
- 배열과 컬렉션의 차이
    - 배열: 고정 크기 이상의 객체 관리 불가능
            배열의 중간에 객체가 삭제되면 응용 프로그램에서 자리를 옮겨야 함
    - 컬렉션: 가변 크기로서 객체의 개수를 염려할 필요X
            컬렉션 내의 한 객체가 삭제되면 컬렉션이 자동으로 자리를 옮겨 줌
#### 컬렉션의 특징
- 제네릭
    - 특정 타입만 다루지 않고, 여러 종류의 타입으로 변신할 수 있도록 클래스나 메소드를 일반화시키는 기법
    - 틀래스나 인터페이스 이름에 <E>, <K>, <V> 등 타입 매개변수 포함
